...
//  also need to update the gcal to store the MealTypeID rather than MealType for consistency
        ...
        switch (mealType.toLowerCase()) {
            ...
            case "lunch": mealColor = "#1ab5ac"; break;
    ...
    function handleEventReceive(info) {
        ...
        if (!mealId || !servings || !mealType) {
            console.error("Missing meal properties");
            return;
        }
   ...
   // }
          ...
          if (!response.items || response.items.length === 0) {
            console.warn("No events found in response");
                Swal.fire({
                    ...
                    text: "Let's schedule some delicous meals!",
                    icon: "info",
    ...
    function saveMealPlacement(mealId, start, end, event, servings, mealType) {
        const payload = {
            summary: event.title,
            description: `MealID:${mealId}|Servings:${servings}|MealType:${mealType}`,
            start: { date: start },
            end: { date: end },
            extendedProperties: {
                private: {
